...
namespace ams::capsrv {
    ...
    Result AlbumControlService::CheckApplicationIdRegistered(u64 appId) {
        WriteLogFile("Control", "CheckApplicationIdRegistered: applicationId(%ld)", appId);
        return ResultSuccess();
    }

    Result AlbumControlService::GenerateCurrentAlbumFileId(sf::Out<FileId> out, u64 appId, ContentType type) {
        WriteLogFile("Control", "GenerateCurrentAlbumFileId: applicationId(%ld), type(%hhd)", appId, type);
        return ResultSuccess();
    }

    Result AlbumControlService::GenerateApplicationAlbumEntry(sf::Out<ApplicationEntry> out, u64 appId, ContentType type) {
        WriteLogFile("Control", "GenerateApplicationAlbumEntry: applicationId(%ld), type(%hhd)", appId, type);
        return ResultSuccess();
    }
    
    Result AlbumControlService::SaveAlbumScreenShotFile(sf::InBuffer buffer, const FileId &fileId) {
        WriteLogFile("Control", "SaveAlbumScreenShotFile: bufferSize(%ld), applicationId(%s)", buffer.GetSize(), fileId.AsString().c_str());
        return ResultSuccess();
    }
    ...
    Result AlbumControlService::SaveAlbumScreenShotFileEx(sf::InNonSecureBuffer buffer, const FileId &fileId, u64 unk0, u64 unk1, u64 unk2) {
        WriteLogFile("Control", "SaveAlbumScreenShotFileEx: bufferSize(%ld), applicationId(%s), %ld:%ld:%ld", buffer.GetSize(), fileId.AsString().c_str(), unk0, unk1, unk2);
        return ResultSuccess();
    }
    
    Result AlbumControlService::SetOverlayScreenShotThumbnailData(sf::InNonSecureBuffer buffer, const FileId &fileId) {
        WriteLogFile("Control", "SetOverlayScreenShotThumbnailData: bufferSize(%ld), applicationId(%s)", buffer.GetSize(), fileId.AsString().c_str());
        return ResultSuccess();
    }
    
    Result AlbumControlService::SetOverlayMovieThumbnailData(sf::InNonSecureBuffer buffer, const FileId &fileId) {
        WriteLogFile("Control", "SetOverlayMovieThumbnailData: bufferSize(%ld), applicationId(%s)", buffer.GetSize(), fileId.AsString().c_str());
        return ResultSuccess();
    }
